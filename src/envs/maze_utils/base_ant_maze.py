...
class BaseAntMazeEnv(MazeEnv, EzPickle):
    metadata = {
        "render_modes": [
            "human",
            "rgb_array",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # MuJoCo 描画コンテキストの作成
        self.context = mujoco.MjrContext(self.ant_env.model, mujoco.mjtFontScale.mjFONTSCALE_150)

        # カメラ設定
        self.camera = mujoco.MjvCamera()
        mujoco.mjv_defaultCamera(self.camera)
        # シーン設定
        self.scene = mujoco.MjvScene(self.ant_env.model, maxgeom=1000)
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        ...
        self.camera.lookat[:2] = self.ant_env.data.qpos[:2]
        self.camera.lookat[2] = 0
        self.camera.distance = 16.0  # 必要に応じて調整
