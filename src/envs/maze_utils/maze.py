...
from .maps import U_MAZE_COLOR, MEDIUM_MAZE_COLOR, LARGE_MAZE_COLOR, NORMAL, RED, GREEN, BLUE
        ...
        for i in range(maze.map_length):
            for j in range(maze.map_width):
                ...
                if struct == 1:  # Unmovable block.
                    if maze_color_map is not None:
                        color = maze_color_map[i][j]
                    else:
                        color = NORMAL
                    if color == NORMAL:
                        rgba = "0.7 0.5 0.3 1.0"
                    else:
                        rgba = f'{1.0 if color==RED else 0.0} {1.0 if color==GREEN else 0.0} {1.0 if color==BLUE else 0.0} 1.0'
...
class MazeEnv(GoalEnv):
    def __init__(
        ...
        maze_color_map: List[List[Union[str]]] = U_MAZE_COLOR,
        ...
        **kwargs,
        ...
        self.maze, self.tmp_xml_file_path = Maze.make_maze(
            agent_xml_path, maze_map, maze_color_map, maze_size_scaling, maze_height
        )
